...
const postEmployeeCon = async (req, res) => {
    const { first_name, last_name , position, contact, department_id } = req.body;
    // Validate input
    if (!first_name || !last_name || !position || !contact || !department_id) {
      return res.status(400).json({ message: 'Missing required fields: full_name, position, contact, or department_id' });
    }
    try {
      // Insert the new employee into the database
      await insertEmployee(first_name, last_name , position, contact, department_id);
...
const patchEmployeeCon = async (req, res) => {
    const {first_name, last_name, position, contact, department_id, employee_id} = req.body;
    // Validate input
    if (!first_name ||!last_name || !position || !contact || !department_id) {
      return res.status(400).json({ message: 'Missing required fields' });
    }
    try {
      await updateEmployee(first_name, last_name, position, contact, department_id, employee_id, req.params.employee_id);
